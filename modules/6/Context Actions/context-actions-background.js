...
class ContextActions extends BackgroundModule {
    ...
    constructor(host) {
        super(host, ContextActions.metadata.siemplifyVersion, ContextActions.metadata.name);
        this.menuTree = {};
    }
    ...
    async executeAction(info, tab, contextAction) {
        // Get variables
        let textSelection = await browser.tabs.executeScript({file: `modules/${ContextActions.metadata.siemplifyVersion}/Context Actions/get-selection.js`});
        textSelection = textSelection[0];
        let environment = await browser.tabs.executeScript({file: `modules/${ContextActions.metadata.siemplifyVersion}/Context Actions/get-environment.js`});
    ...
    processEnvironment(environment) {
        ...
        if (regex != "") {
            ...
            result = (match) ? match[1] : environment;
        }
        else {
            result = environment
        }
