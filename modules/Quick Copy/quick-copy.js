...
var QuickCopy = (function () {
	var contextTitleSelector = ["p.description", "span.description"];
	var caseDescriptionSelector = "div.description-case";
	...
	var enable = function () {
		...
		descriptionObserver = new PersistentObserver(caseDescriptionSelector, { characterData: true }, handleCaseIdSummaries, handleCaseIdAttach);
		...
		let description = document.querySelector(caseDescriptionSelector);
		if (description) description.addEventListener("click", copyCaseIdHandler);
	...
	var disable = function () {
		...
		let description = document.querySelector(caseDescriptionSelector);
		if (description) description.removeEventListener("click", copyCaseIdHandler);
		let title = document.querySelector(contextTitleSelector);
		if (title) title.removeEventListener("click", copyContextTitleHandler);
	};

	// ======================= Case ID copy handler =======================
	...
	/**
		Copy handler for header description
		this -> clicked element
	**/
	var copyCaseIdHandler = function() {
		copyToClipboard(this.textContent.match(/^ID (\d+) |/)[1]);
