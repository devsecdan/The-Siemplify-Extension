...
var DetectLastAlert = (function () {
	...
	**/
	var addListeners = function() {
		if (!browser.runtime.onMessage.hasListener(handleAlertingEnvironment)) browser.runtime.onMessage.addListener(handleAlertingEnvironment);
		if (!browser.runtime.onMessage.hasListener(handleCheckEnvironments)) browser.runtime.onMessage.addListener(handleCheckEnvironments);
		if (!browser.runtime.onMessage.hasListener(handleDismissAlert)) browser.runtime.onMessage.addListener(handleDismissAlert);
	}

	var removeListeners = function() {
		browser.runtime.onMessage.removeListener(handleAlertingEnvironment);
		browser.runtime.onMessage.removeListener(handleCheckEnvironments);
		browser.runtime.onMessage.removeListener(handleDismissAlert);
	}
	...
	**/
	var getAlerts = function() {
		browser.runtime.sendMessage( { "getAlerts": true } )
		.then(response => {
	...
	**/
	var dismissAlert = function(environment) {
		browser.runtime.sendMessage( { "dismissAlert": environment } );
	}
	...
	**/
	var handleSearchResponse = function(request, environment) {
		...
		if (response) {
			...
			if (responseCases.length == 0) {
				...
				browser.runtime.sendMessage( { "nonAlertingEnvironment": environment } );
			}
		}
	}
	...
	var sendEnvironmentAlert = function(request, environment) {
		...
		browser.runtime.sendMessage( { alertingEnvironment: environment, lastAlertTime: time} );
	}
